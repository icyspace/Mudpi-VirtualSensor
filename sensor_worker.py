...
from sensors.pi.precipitation_sensor import (PrecipitationSensor)
        ...
        def init(self):
                for sensor in self.config['sensors']:
                        if sensor.get('type', None) is not None:
                                ...
                                # Open Weather API Key is specific to the precipitation modules this is required
                                if sensor.get('openweather_api_key'):
                                        sensor_kwargs['openweather_api_key'] = str(sensor.get('openweather_api_key'))

                                # Lat and Long are specific to the precipitation modules and are optional
                                if sensor.get('latlong'):
                                        sensor_kwargs['latlong'] = str(sensor.get('latlong'))

                                # forecasted_hours are specific to the precipitation modules and are optional
                                if sensor.get('forecasted_hours'):
                                        sensor_kwargs['forecasted_hours'] = str(sensor.get('forecasted_hours'))
